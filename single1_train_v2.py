...
from network_v2 import build_dataset, build_decoder, build_encoder, VectorQuantizerEMA

os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'

path_workspace = './ws/'
path_embedding_data = "embeding/training/"
...
with tf.compat.v1.variable_scope(experiment_name):
    ...
    ################# Normalize images ###########################
    bgr_y = tf.compat.v1.placeholder(tf.uint8, shape=(image_size, image_size, 3))
    ...
    x = tf.compat.v1.placeholder(tf.float32, shape=(None, image_size, image_size, ci))
    pose_label = tf.compat.v1.placeholder(tf.int32, shape=(None, 1))
    y_gt_ae = tf.compat.v1.placeholder(tf.float32, shape=(None, image_size, image_size, co))

    ppixel_edge_w_ae = tf.compat.v1.placeholder(dtype=tf.float32, shape=(None, image_size, image_size, 1))
    gprior_decay = tf.compat.v1.placeholder(tf.float32, shape=None)
    gprior_temperature = tf.compat.v1.placeholder(tf.float32, shape=None)
    gprior_lambda_reconst_cost = tf.compat.v1.placeholder(tf.float32, shape=None)
    ...
    with tf.compat.v1.variable_scope('embedding_lookup'):
        pose_train_retr = pose_embeds(z, decay=gprior_decay, temperature=gprior_temperature,
                                       encoding_1nn_indices=pose_label, encodings=None, is_training=True)
    ...
    if co in [3, 4]:
        ...
        bootstrap_ratio = 4
        y_gt_flat = tf.compat.v1.layers.flatten(reconst_target_bgr)
        print(reconst_target_bgr.shape)
        print(y_recon_ae['x_bgr'].shape)
        y_reconae_flat = tf.compat.v1.layers.flatten(y_recon_ae['x_bgr'])
        l2_ae = tf.compat.v1.losses.mean_squared_error(
            y_reconae_flat,
            y_gt_flat,
        ...
        tf.compat.v1.summary.scalar('recons_loss_ae_bgr', recon_error_ae_bgr)
    ...
    if co in [1, 4]:
        ...
        x_flat = tf.compat.v1.layers.flatten(y_recon_ae['x_edge'])
        reconstruction_label_flat = tf.compat.v1.layers.flatten(reconst_target_edge)

        loss_weight_flat = tf.compat.v1.layers.flatten(ppixel_edge_w_ae)
        ...
        tf.compat.v1.summary.scalar('recons_loss_ae_edge', recon_error_ae_edge)

    total_loss += recon_error_ae * 1.
    total_loss += (pose_train_retr["loss"]) / gprior_lambda_reconst_cost
    pose_cosine_loss = tf.reduce_mean(
        (tf.nn.l2_normalize(tf.stop_gradient(pose_train_retr["quantize_1nn"]), axis=1) -
         tf.nn.l2_normalize(tf.stop_gradient(z), axis=1)) ** 2)
    tf.compat.v1.summary.histogram('pose z', z)

    tf.compat.v1.summary.scalar('recons_loss_ae', recon_error_ae)
    tf.compat.v1.summary.scalar('pose_ce_loss', pose_train_retr["loss"])
    tf.compat.v1.summary.scalar('pose_cosine_err', pose_cosine_loss)
    tf.compat.v1.summary.scalar('temperature', gprior_temperature)
    tf.compat.v1.summary.scalar('lambda_reconst_cost', gprior_lambda_reconst_cost)

    mean, var = tf.nn.moments(pose_embeds.embeddings, 1)
    tf.compat.v1.summary.histogram('pose codebook mean', mean)
    tf.compat.v1.summary.histogram('pose codebook var', var)
    tf.compat.v1.summary.histogram('pose codebook', pose_embeds.embeddings)
    ...
    saver = tf.compat.v1.train.Saver(save_relative_paths=True, max_to_keep=50)
...
gpu_options = tf.compat.v1.GPUOptions(allow_growth=True, per_process_gpu_memory_fraction=0.9)
